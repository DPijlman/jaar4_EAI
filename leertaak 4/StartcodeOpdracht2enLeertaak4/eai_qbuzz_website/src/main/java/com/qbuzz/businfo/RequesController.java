...
import com.qbuzz.soapserver.BerichtLogging;
import com.qbuzz.soapserver.BusManager;
import com.qbuzz.soapserver.ETAPersistence;
...
public class RequesController {
	...
	@GetMapping("QBUZZ_Info/logging")
	public String logging(@RequestParam(defaultValue = "0") String vanaf, 
						  @RequestParam(defaultValue = "10") String tot,
			              Model model) {
		ArrayList<BerichtLogging> results = BusManager.getLoggingInfo(Integer.parseInt(vanaf), Integer.parseInt(tot));
		model.addAttribute("results", results);
		model.addAttribute("aantal", ETAPersistence.getAantalBerichten());
		return "logging";
	}
}
